...
export const SettingsNotificationPreferencesView = ({
  ...
  const setType = (type: NotificationType, enabled: boolean) => {
    setStatus(null);
    setPreferences((prev) => ({ ...prev, types: { ...prev.types, [type]: enabled } }));
  };
  ...
  return (
    <div className="flex max-w-2xl flex-col gap-6 p-4">
      ...
      <section className="flex flex-col gap-4">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex flex-col gap-0.5">
            ...
            <p className="text-muted-foreground text-sm">{t("browserDescription")}</p>
            <p className="text-muted-foreground/80 text-xs">{permissionMessage}</p>
          </div>
          <Switch
            id="pref-browser"
            ...
            onCheckedChange={(checked) => {
              setStatus(null);
              setPreferences((prev) => ({ ...prev, enabledInBrowser: checked }));
            }}
          />
        </div>
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <Switch
            ...
            onCheckedChange={(checked) => {
              setStatus(null);
              setPreferences((prev) => ({ ...prev, enabledInEmail: checked }));
            }}
          />
        </div>
      </section>
      ...
      <div className="flex items-center gap-3">
        <Button type="button" onClick={() => save.mutate()} disabled={!data || save.isPending}>
