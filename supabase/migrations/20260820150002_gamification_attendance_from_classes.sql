-- public.sync_user_achievements() computed perfect-attendance from
-- public.lessons.attendance_status, which just moved to public.classes in
-- 20260820150001_lessons_class_split.sql. Applied migrations are never
-- edited, so this reissues the whole function with only that block changed:
-- it now reads public.classes, and counts "attended" against occurrences
-- that actually have a recorded attendance_status (not against every row —
-- future/untouched occurrences are NULL, not absent).
...
comment on function public.sync_user_achievements() is
  ...
  'perfect-attendance reads public.classes (moved from public.lessons in '
  '20260820150001_lessons_class_split.sql). Idempotent and safe to call on '
  'every dashboard/achievements load.';
