...
export const FlashcardsReview = async ({ searchParams }: FlashcardsReviewProps) => {
  const { subjectId, lessonId } = await searchParams;

  const { data } = await Actions.Flashcards.getDueQueue({ subjectId, lessonId });

  const backHref = lessonId ? `/lessons/detail/${lessonId}` : "/flashcards/deck";
