...
export function WeeklySummaryCard({ summary }: WeeklySummaryCardProps) {
  ...
  const chartData = summary.dayBreakdown.map((day) => ({
    label: new Intl.DateTimeFormat(locale, { weekday: "short" }).format(new Date(day.date)),
    minutes: day.minutes,
  }));

  const description =
    summary.targetMinutes > 0
      ? t("subtitle", { minutes: summary.totalMinutes, target: summary.targetMinutes })
      ...
      <div style={{ height: 200 }}>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
            ...
            />
            <Bar dataKey="minutes" radius={[4, 4, 0, 0]} style={{ fill: "var(--primary)" }} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </ChartCard>
